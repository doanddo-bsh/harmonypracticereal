...
//
// baseNote 로 3음이 뽑혔을 때(70% 확률)는 제거했다가 다시 3음을 넣는
// 꼴이라 우연히 3화음 구성이 보존되지만, baseNote 로 근음이나 5음이
// 뽑혔을 때(합쳐서 30% 확률)는 그 음이 결과에서 사라지고 3음이 중복으로
// 들어간다. 그런데 note4Shuffle(문제로 나가는 4성부)에는 baseNote 가
// 그대로 포함되므로, "문제의 모든 음은 원화음에 있어야 한다" 는 불변식이
// 깨진다.
//
...
void main() {
  group('장조 문제 생성 불변식', () {
    ...
    test('neapolitanProblem (나폴리화음) — 알려진 버그로 인해 완화된 검증', () {
      for (var i = 0; i < 200; i++) {
        final (answer, problem, tonality, original, name) =
            neapolitanProblem();
